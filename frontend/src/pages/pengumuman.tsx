...
import { Megaphone, Calendar, Users, ArrowLeft, Download, CheckCircle2, Award, FileText, Briefcase, GraduationCap } from 'lucide-react';
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 mb-16 text-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50/50 border border-blue-100 mb-6">
              ...
              <span className="text-[#0D278D] text-sm font-bold tracking-widest uppercase">Pusat Informasi</span>
            </div>
            ...
            <p className="text-gray-500 text-lg max-w-2xl mx-auto leading-relaxed">
              Selamat kepada para kandidat yang telah dinyatakan lulus. Berikut adalah daftar resmi hasil akhir seleksi penerimaan.
            </p>
          </motion.div>
        </section>
          ...
          <div className="bg-white/60 backdrop-blur-md rounded-[3rem] p-4 sm:p-8 md:p-10 border border-white shadow-[0_30px_60px_-15px_rgba(0,0,0,0.03)]">
            ...
            {selectedId === null && (
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10 pb-6 border-b border-gray-100 px-2">
                <div>
                  <h2 className="text-2xl font-bold text-[#0D278D]">Daftar Pengumuman</h2>
                  <p className="text-sm text-gray-500 font-medium mt-1">
                    Menampilkan <span className="text-[#FEB700] font-bold">{announcements.length}</span> pengumuman terbaru
                  </p>
                </div>
              </div>
            )}
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-3 mb-4">
                          <span className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 border ${
                            item.kategori === 'Konsultan Individu' ? 'bg-[#FEB700]/10 text-[#FEB700] border-[#FEB700]/20' : 'bg-blue-50 text-[#0D278D] border-blue-100'
                          }`}>
                        ...
                        <div className="flex flex-wrap items-center gap-6">
                          ...
                          <div className="hidden sm:flex items-center gap-4 text-sm text-gray-500 font-medium border-l border-gray-200 pl-6">
                            <span className="flex items-center gap-1.5"><Users size={16} /> {item.totalPendaftar} Pelamar</span>
                            <span className="flex items-center gap-1.5 text-[#0D278D] font-bold"><Award size={16} /> {item.lulus.length} Diterima</span>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <div className="p-8 md:p-12 border-b border-gray-100 bg-[#fafafa]">
                      ...
                      <h2 className="text-3xl md:text-4xl font-extrabold text-[#0D278D] mb-6 leading-tight">
                        ...
                        <span className="text-[#FEB700]">{selectedData?.posisi}</span>
                      </h2>
                      
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                        <p className="text-gray-500 font-medium">
                          Berdasarkan hasil keputusan panitia, ditetapkan <span className="font-bold text-[#0D278D] px-2 py-1 bg-blue-50 rounded-md border border-blue-100">{selectedData?.lulus.length} orang</span> kandidat yang dinyatakan Lulus.
                        </p>
                        
                        <button 
                          onClick={() => alert("Mendownload file SK_Pengumuman_" + selectedData?.posisi.substring(0,5) + ".pdf")}
                        ...
                        {selectedData?.lulus.map((person, index) => (
                          ...
                          >
                            <div className="flex items-center gap-5">
                              ...
                              <div>
                                <h5 className="font-bold text-[#0D278D] text-lg group-hover:text-[#FEB700] transition-colors">{person.nama}</h5>
                                <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider mt-0.5">
                                  Nomor Induk: <span className="text-gray-400">{person.nik}</span>
                                </p>
                              </div>
                            </div>
