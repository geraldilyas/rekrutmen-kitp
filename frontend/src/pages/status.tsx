...
import { Briefcase, Calendar, ChevronDown, CheckCircle2, Clock, XCircle, GraduationCap, FileText, Search, Send, Megaphone, Brain, Users, CircleDot } from 'lucide-react';
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 mb-16 text-center">
          ...
          >
            ...
            <p className="text-gray-500 text-lg max-w-2xl mx-auto leading-relaxed">
              Lihat perkembangan proses seleksi Anda secara transparan dan real-time.
            </p>
          </motion.div>
        </section>
        ...
        <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white rounded-[2.5rem] p-6 sm:p-10 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 border-b border-gray-50 pb-8">
              <div>
                <h2 className="text-2xl font-bold text-[#0D278D]">Riwayat Lamaran</h2>
                  ...
                  >
                    ...
                    >
                      <div className="flex-1 w-full">
                        ...
                        <h3 className="text-lg font-bold text-[#0D278D] mb-2 leading-tight">{job.posisi}</h3>
                      ...
                      <div className="flex items-center justify-between w-full md:w-auto gap-4">
                        <div className={`px-4 py-2 rounded-full text-xs font-semibold flex items-center gap-2 ${
                          job.currentStep === 6 
                            ? (job.isRejected ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600')
                            : 'bg-blue-50 text-[#0D278D]'
                        }`}>
                          {job.currentStep === 6 
                            ? (job.isRejected ? <XCircle size={14} /> : <CheckCircle2 size={14} />)
                            : <Clock size={14} className="animate-spin-slow" />
                          }
                          {job.statusText}
                        </div>
                        
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-transform duration-300 text-gray-400 bg-gray-50 ${expandedId === job.id ? 'rotate-180 bg-blue-50 text-[#0D278D]' : ''}`}>
                          <ChevronDown size={18} />
                        </div>
                      </div>
                    </div>
                                ...
                                <motion.div 
                                  ...
                                  className={`absolute top-0 left-0 h-[4px] rounded-full transition-all duration-700 ease-out ${
                                    job.isRejected && job.currentStep === 6 
                                      ? 'bg-red-500' 
                                      : (job.currentStep === 6 && !job.isRejected ? 'bg-green-500' : 'bg-[#0D278D]')
                                  }`}
                                />
                              </div>
                              ...
                              <div className="flex flex-col md:flex-row justify-between gap-8 md:gap-0 relative z-10 w-full">
                                {timelineSteps.map((step) => {
                                  ...
                                  if (isEndAccepted) {
                                    ...
                                    circleClass = 'bg-[#0D278D] border-[#0D278D] text-white shadow-md shadow-blue-900/10';
                                    titleClass = 'text-[#0D278D] font-bold';
                                    IconCmp = CheckCircle2;
                                  } else if (isCurrent && !isEndRejected) {
                                    // AKTIF SEDANG BERJALAN (KUNING)
                                    circleClass = 'bg-white border-[#FEB700] text-[#FEB700] ring-4 ring-yellow-50 shadow-md';
                                    titleClass = 'text-[#0D278D] font-bold';
                                    IconCmp = step.icon; 
                                  ...
                                  return (
                                    <div key={step.id} className="flex flex-row md:flex-col items-center md:items-center gap-4 md:gap-3 w-full md:w-[16.66%] shrink-0">
                                      
                                      {/* Lingkaran (Bulatan) Timeline */}
                                      <div className={`w-12 h-12 rounded-full border-[3px] flex items-center justify-center transition-all duration-500 z-10 ${circleClass}`}>
                                        <IconCmp size={20} strokeWidth={isCurrent && !isPast ? 2.5 : 2} />
                                      </div>

                                      {/* Teks Keterangan */}
                                      <div className="text-left md:text-center w-full md:px-2">
                                        <h3 className={`text-sm md:text-[13px] ${titleClass}`}>
                                          {step.id === 6 && isEndRejected ? 'Ditolak' : 
                                           step.id === 6 && isEndAccepted ? 'Diterima' : step.label}
                                        </h3>
            ...
            {filteredJobs.length === 0 && (
              <div className="text-center py-20 border-2 border-dashed border-gray-100 rounded-3xl mt-6">
                ...
                <h3 className="text-[#0D278D] font-bold text-lg">Tidak ada lamaran</h3>
                <p className="text-gray-400 text-sm mt-1">Anda belum memiliki lamaran di kategori ini.</p>
              </div>
            )}
