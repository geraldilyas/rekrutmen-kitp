...
import { Filter, Briefcase, Calendar, ChevronDown, CheckCircle2, Clock, XCircle, Send, GraduationCap, FileText } from 'lucide-react';
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 mb-16 text-center">
          ...
          >
            ...
            <p className="text-gray-500 text-lg max-w-2xl mx-auto leading-relaxed">
              Lihat perkembangan proses seleksi Anda secara real-time. Kami mengedepankan transparansi dalam setiap tahapan rekrutmen.
            </p>
          </motion.div>
        </section>
        ...
        <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap items-center gap-3 mb-4">
                        <span className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 ${
                          job.kategori === 'Konsultan Individu' ? 'bg-[#FEB700]/10 text-[#FEB700]' : 'bg-blue-50 text-[#0D278D]'
                        }`}>
                      ...
                      {/* Judul & Deskripsi */}
                      <h3 className="text-2xl font-bold text-[#0D278D] mb-2">{job.posisi}</h3>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {job.pendidikan.map((edu, index) => (
                          <span key={index} className="w-7 h-7 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center text-[10px] font-bold text-[#0D278D]">
                            {edu}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-4 border-t md:border-t-0 md:border-l border-gray-100 pt-6 md:pt-0 md:pl-8">
                      <div className={`px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 shadow-sm ${
                        job.currentStep === 3 
                          ? (job.isRejected ? 'bg-red-50 text-red-600 border border-red-100' : 'bg-green-50 text-green-600 border border-green-100')
                          : 'bg-blue-50 text-[#0D278D] border border-blue-100'
                      }`}>
                        {job.currentStep === 3 
                          ? (job.isRejected ? <XCircle size={18} /> : <CheckCircle2 size={18} />)
                          : <Clock size={18} className="animate-spin-slow" />
                        }
                        {job.statusText}
                      </div>
                      
                      <button className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 shadow-sm ${expandedId === job.id ? 'bg-[#0D278D] text-white rotate-180' : 'bg-gray-50 text-[#0D278D] border border-gray-200 hover:bg-[#FEB700] hover:border-[#FEB700]'}`}>
                        <ChevronDown size={24} />
                      </button>
                    </div>
                  </div>
                      ...
                      >
                        <div className="p-8 md:p-12">
                          ...
                          <div className="relative px-4 md:px-10">
                            ...
                            <motion.div 
                              initial={{ width: 0 }}
                              animate={{ width: `${((job.currentStep - 1) / 2) * 100}%` }}
                            ...
                            <motion.div 
                              initial={{ height: 0 }}
                              animate={{ height: `${((job.currentStep - 1) / 2) * 100}%` }}
                              transition={{ duration: 0.8, ease: "easeOut", delay: 0.1 }}
                            ...
                            <div className="flex flex-col md:flex-row justify-between gap-8 md:gap-0 relative z-10">
                              {timelineSteps.map((step) => {
                                ...
                                const isEndRejected = job.currentStep === 3 && job.isRejected && step.id === 3;
                                const isEndAccepted = job.currentStep === 3 && !job.isRejected && step.id === 3;

                                // Style penentuan Node
                                let dotColor = 'bg-gray-200 text-gray-400 border-white';
                                let pulseEffect = '';
                                
                                if (isPast || isEndAccepted) {
                                  dotColor = 'bg-[#0D278D] text-white border-white';
                                } else if (isCurrent && !isEndRejected) {
                                  dotColor = 'bg-[#FEB700] text-[#0D278D] border-white';
                                  pulseEffect = 'ring-4 ring-[#FEB700]/30 animate-pulse';
                                } else if (isEndRejected) {
                                  dotColor = 'bg-red-500 text-white border-white';
                                }
                                ...
                                return (
                                  <div key={step.id} className="flex md:flex-col items-center md:justify-start gap-4 md:gap-4 md:w-1/3">
                                    {/* Circle Node */}
                                    <div className={`w-12 h-12 md:w-14 md:h-14 rounded-full flex shrink-0 items-center justify-center border-[4px] shadow-md transition-all duration-500 z-10 ${dotColor} ${pulseEffect}`}>
                                      <StepIcon size={22} strokeWidth={2.5} />
                                    </div>
                                    
                                    {/* Text Content */}
                                    <div className="text-left md:text-center mt-0 md:mt-2">
                                      <h3 className={`text-sm md:text-base font-bold mb-1 transition-colors duration-300 ${isPast || isEndAccepted || isCurrent ? 'text-[#0D278D]' : 'text-gray-400'}`}>
                                        {step.id === 3 && isEndRejected ? 'Ditolak' : 
                                         step.id === 3 && isEndAccepted ? 'Diterima' : step.label}
                                      </h3>
                                      <p className="text-xs text-gray-500 font-medium">
                                        {isCurrent ? 'Tahap saat ini' : isPast ? 'Telah dilalui' : isEndRejected ? 'Mohon maaf' : 'Belum dimulai'}
                                      </p>
                                    </div>
                                  </div>
                                );
          ...
          {filteredJobs.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-gray-500 text-lg font-medium">Belum ada lamaran di kategori ini.</p>
            </div>
          )}
        </section>
      </main>
