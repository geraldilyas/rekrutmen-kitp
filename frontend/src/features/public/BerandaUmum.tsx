...
const Lowongan: React.FC = () => {
  ...
  const fetchJobs = async () => {
    try {
      ...
      yearsSet.add(new Date().getFullYear().toString()); // Tetap masukkan tahun ini sebagai fallback
  ...
  const filteredJobs = jobs
    .filter((job) => {
      ...
      if (filterYear !== "all" && startObj && endObj) {
        ...
        // Lolos jika range pendaftaran melewati atau berada di tahun yang dipilih
        matchesYear = (selectedYearInt >= startYear && selectedYearInt <= endYear);
      }

      // 2. Validasi Filter Bulan Terpilih (Disinkronkan dengan Filter Tahun)
      ...
      if (filterMonth !== "all" && startObj && endObj && !isNaN(startObj.getTime()) && !isNaN(endObj.getTime())) {
        ...
        if (filterYear !== "all") {
          // Jika tahun dispesifikkan, pastikan bulan masuk akal di tahun tersebut
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 flex-1 w-full" onClick={(e) => e.stopPropagation()}>
            ...
            {/* 🛠️ 4. WIDGET DROPDOWN FILTER TAHUN (BARU & DINAMIS) */}
