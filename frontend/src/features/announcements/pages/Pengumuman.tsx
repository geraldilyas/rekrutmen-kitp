...
// --- DATA ---
const announcements = [
  { id: 1, kategori: "Tenaga Pendukung", jurusan: "Teknik Sipil", posisi: "Tenaga Pendamping Masyarakat (TPM) - P3-TGAI", deskripsi: "Melaksanakan pendampingan kepada P3A/GP3A/IP3A dalam aspek teknis, administratif, dan sosial pada kegiatan Percepatan Peningkatan Tata Guna Air Irigasi.", pendidikan: ["S1", "D3"], tanggalTutup: "20 April 2026", totalPendaftar: 245, totalLulus: 5 },
  { id: 2, kategori: "Konsultan Individu", jurusan: "IT Support", posisi: "Software Engineer (Full-Stack)", deskripsi: "Mengembangkan dan memelihara sistem informasi manajemen sumber daya air berbasis web and mobile yang terintegrasi.", pendidikan: ["S1"], tanggalTutup: "15 April 2026", totalPendaftar: 128, totalLulus: 2 },
  { id: 3, kategori: "Tenaga Pendukung", jurusan: "Administrasi", posisi: "Petugas Administrasi Satker", deskripsi: "Mendukung pengelolaan administrasi perkantoran, kearsipan, dan penyusunan laporan rutin pada satuan kerja Balai.", pendidikan: ["S1", "D3", "SMA"], tanggalTutup: "10 April 2026", totalPendaftar: 412, totalLulus: 3 },
];
        ...
        <div className="max-w-5xl mx-auto px-4 text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 border border-white/20 mb-6 backdrop-blur-sm">
              ...
              <span className="text-white text-[11px] font-bold tracking-widest uppercase">Pusat Informasi</span>
            </div>
            
            <h1 className="text-4xl md:text-6xl font-extrabold text-white mb-6">
              Pengumuman <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FEB700] to-[#ffe066]">Hasil Seleksi</span>
            </h1>
            
            {/* Subhead Hero */}
            <p className="text-blue-100/80 text-[15px] md:text-base max-w-2xl mx-auto leading-relaxed">
              Informasi resmi hasil seleksi akhir, daftar kandidat yang dinyatakan diterima, serta penutupan tahapan rekrutmen.
            </p>
          </motion.div>
        </div>
      </div>
        ...
        >
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-2">
              Menampilkan <span className="text-[#FEB700] font-bold">{filteredData.length}</span> pengumuman aktif
            </p>
          </div>
                  ...
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-3 mb-4">
                       <span className={`text-[11px] font-bold uppercase tracking-wider flex items-center gap-1.5 px-2.5 py-1 rounded-md ${
                                                    item.kategori === "Konsultan Individu" ? "bg-amber-50 text-[#FEB700]" : "bg-blue-50 text-[#0D278D]"
                                                                             }`}>
                       ...
                       <span className="text-[11px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
                         <Clock size={14} className="text-[#FEB700]" /> {item.tanggalTutup}
                       </span>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-6">
                      <div className="flex items-center gap-2">
                        ...
                        {item.pendidikan.map((edu, idx) => (
                          <span key={idx} className="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-[10px] font-black text-[#0D278D] border border-gray-100">
                            {edu}
                          </span>
                        ))}
                      </div>
                      <div className="flex items-center gap-6 text-sm font-bold text-gray-400 border-l border-gray-100 pl-6">
                          <span className="flex items-center gap-1.5"><Users size={16}/> {item.totalPendaftar} Pendaftar</span>
                          <span className="flex items-center gap-1.5 text-[#0D278D]"><Award size={16}/> {item.totalLulus} Diterima</span>
                      </div>
                    </div>
                  </div>
